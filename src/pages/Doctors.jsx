...
const Doctors = () => {
  ...
  return (
    <div>
      ...
      <div className="flex md:flex-row flex-col items-start gap-2 mt-10">
        {/* -------------- Navigation ----------- */}
        ...
        <div className="flex mt-10 flex-wrap lg:gap-4 gap-2 md:justify-start items-center justify-center">
          {
             ...
             : filterDoc.map( (item, index) => (
                 <div key={index} onClick={() => {navigate(`/appointment/:${item._id}`); scrollTo(0,0)}} className="rounded-2xl flex flex-col overflow-hidden relative shadow-md hover:translate-y-[-10px] transition-all duration-500 cursor-pointer">
                     <img src={item.image} className=' pt-5 lg:px-8 px-7 mx-auto bg-blue-400/20 ' alt="" />
